...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/08/21 13:49:11 by glima-de          #+#    #+#             */
/*   Updated: 2021/08/21 13:49:11 by glima-de         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	applyTest(char *str, int n)
{
	...
	copy(str, r_ori);
	copy(str, r_cus);
	so = strlcat(r_ori, str, n);
	sc = ft_strlcat(r_cus, str, n);
	while (r_cus[i] || r_ori[i])
	{
		if (r_cus[i] != r_ori[i] || so != sc)
		{
			printf("ft_strlcat: KO! ");
			printf("sended *%s*, expected \n*%s*, got \n*%s*", str, r_ori, r_cus);
